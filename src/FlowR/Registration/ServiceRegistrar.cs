...
public static class ServiceRegistrar
{
    ...
    {
        MaxGenericTypeParameters = configuration.MaxOpenGenericRequestParamCount;
        MaxTypesClosing = configuration.MaxOpenGenericRequestTypeCount;
        MaxGenericTypeRegistrations = configuration.MaxOpenGenericHandlerCount;
        RegistrationTimeout = configuration.GenericRegistrationTimeout;
    }
    ...
    public static void AddFlowRClassesWithTimeout(IServiceCollection services, FlowRServiceConfiguration configuration)
    {
        ...
        using(var cts = timeout > 0 ? new CancellationTokenSource(timeout) : new CancellationTokenSource())
        {
            try
            {
                AddFlowRClasses(services, configuration, cts.Token);
            }
    ...
    public static void AddFlowRClasses(IServiceCollection services, FlowRServiceConfiguration configuration, CancellationToken cancellationToken = default)
    {   
        ...
        if (configuration.AutoRegisterProcessors)
        {
        ...
        foreach (var multiOpenInterface in multiOpenInterfaces)
        {
            ...
            var concretions = assembliesToScan
                ...
                .Where(configuration.TypeFilter)
                .ToList();
    ...
    private static (Type Service, Type Implementation) GetConcreteRegistrationTypes(Type openRequestHandlerInterface, Type concreteGenericTRequest, Type openRequestHandlerImplementation)
    {
        ...
        Type serviceType;
        if (openRequestHandlerInterface.GetGenericArguments().Length == 1)
        {
            serviceType = typeDefinition.MakeGenericType(concreteGenericTRequest);
        }
        else if (concreteTResponse != null)
        {
            serviceType = typeDefinition.MakeGenericType(concreteGenericTRequest, concreteTResponse);
        }
        else
        {
            serviceType = typeDefinition.MakeGenericType(concreteGenericTRequest, typeof(Unit));
        }
    ...
    public static void AddRequiredServices(IServiceCollection services, FlowRServiceConfiguration serviceConfiguration)
    {
        ...
        var notificationPublisherServiceDescriptor = serviceConfiguration.NotificationPublisherType != null
            ...
            : serviceConfiguration.NotificationPublisher != null
                ? new ServiceDescriptor(typeof(INotificationPublisher), serviceConfiguration.NotificationPublisher)
                : new ServiceDescriptor(typeof(INotificationPublisher), typeof(NotificationPublishers.ForeachAwaitPublisher), serviceConfiguration.Lifetime);
