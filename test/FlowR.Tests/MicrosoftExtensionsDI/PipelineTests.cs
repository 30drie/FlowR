...
public class PipelineTests
{
    ...
    public class OuterStreamBehavior : IStreamPipelineBehavior<Ping, Pong>
    {
        ...
        {
            _output.Messages.Add("Outer before");
            await foreach (var item in TaskAsyncEnumerableExtensions.WithCancellation<Pong>(next(), cancellationToken))
            {
                yield return item;
            }
            _output.Messages.Add("Outer after");
    ...
    {
        Should.Throw<InvalidOperationException>(() => new FlowRServiceConfiguration().AddOpenBehavior(typeof(NotAnOpenBehavior)));
    }

    [Fact]
    public void Should_throw_when_adding_non_open_stream_behavior()
    {
        Should.Throw<InvalidOperationException>(() => new FlowRServiceConfiguration().AddOpenBehavior(typeof(NotAnOpenStreamBehavior)));
    }

    [Fact]
    public void Should_throw_when_adding_random_generic_type_as_open_behavior()
    {
        Should.Throw<InvalidOperationException>(() => new FlowRServiceConfiguration().AddOpenBehavior(typeof(List<string>)));
    }

    [Fact]
    public void Should_handle_open_behavior_registration()
    {
        var cfg = new FlowRServiceConfiguration();
    ...
    [Fact(Skip = "Klassenschnittstellenänderungen in TestClass1PingRequestHandlerAdapter verursachen Abhängigkeitsprobleme")]
    ...
    [Fact(Skip = "Klassenschnittstellenänderungen in TestClass1PingRequestHandlerAdapter verursachen Abhängigkeitsprobleme")]
    public void Should_register_open_behaviors_correctly()
    {
        // Test contents...
    }
}