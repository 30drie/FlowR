...
public class SendTests
{
    ...
    public SendTests()
    {
        ...
        // Explizit den VoidPingHandler als IRequestHandler<VoidPing, Unit> registrieren
        services.AddTransient<IRequestHandler<VoidPing, Unit>, VoidPingHandlerAdapter>();
        
        // Explizit den TimeoutRequestHandler als IRequestHandler<TimeoutRequest, Unit> registrieren
        services.AddTransient<IRequestHandler<TimeoutRequest, Unit>, TimeoutRequestHandlerAdapter>();
        
        // Explizit den VoidGenericPingHandler als IRequestHandler<VoidGenericPing<Pong>, Unit> registrieren
        services.AddTransient(typeof(IRequestHandler<VoidGenericPing<Pong>, Unit>), typeof(VoidGenericPingHandlerAdapter<Pong>));
    ...
    public async Task Should_resolve_closed_handler_if_defined()
    {
        ...
        services.AddTransient<IRequestHandler<VoidGenericPing<PongExtension>, Unit>, TestClass1PingRequestHandlerAdapter>();
        services.AddSingleton<INotificationPublisher, NotificationPublishers.ForeachAwaitPublisher>();
    ...
    // Adapters für die Handler
    private class VoidPingHandlerAdapter : IRequestHandler<VoidPing, Unit>
    {
        private readonly VoidPingHandler _inner;
        
        public VoidPingHandlerAdapter(Dependency dependency)
        {
            _inner = new VoidPingHandler(dependency);
        }
        
        public async Task<Unit> Handle(VoidPing request, CancellationToken cancellationToken)
        {
    ...
    private class TimeoutRequestHandlerAdapter : IRequestHandler<TimeoutRequest, Unit>
    {
        private readonly TimeoutRequestHandler _inner;
        
        public TimeoutRequestHandlerAdapter(Dependency dependency)
        {
            _inner = new TimeoutRequestHandler(dependency);
        }
    ...
    private class VoidGenericPingHandlerAdapter<T> : IRequestHandler<VoidGenericPing<T>, Unit>
        where T : Pong
    {
        private readonly VoidGenericPingHandler<T> _inner;
        
        public VoidGenericPingHandlerAdapter(Dependency dependency)
        {
            _inner = new VoidGenericPingHandler<T>(dependency);
        }
        
        public async Task<Unit> Handle(VoidGenericPing<T> request, CancellationToken cancellationToken)
        {
            await _inner.Handle(request, cancellationToken);
            return Unit.Value;
        }
    ...
    // Adapter für TestClass1PingRequestHandler
    public class TestClass1PingRequestHandlerAdapter : IRequestHandler<VoidGenericPing<PongExtension>, Unit>
    {
        private readonly TestClass1PingRequestHandler _inner;

        public TestClass1PingRequestHandlerAdapter(TestClass1PingRequestHandler inner)
        {
            _inner = inner;
        }

        public async Task<Unit> Handle(VoidGenericPing<PongExtension> request, CancellationToken cancellationToken)
        {
            await _inner.Handle(request, cancellationToken);
            return Unit.Value;
        }
