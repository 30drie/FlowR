...
using FlowR.Tests.MicrosoftExtensionsDI;
    ...
    public class GenericRequestHandlerTests : BaseGenericRequestHandlerTests
    {
        ...
        public void ShouldThrowExceptionWhenGenericTypeParametersExceedsMaximum()
        {
            ...
            Should.Throw<ArgumentException>(() =>
            {
                services.AddFlowR(cfg =>
                {
                    cfg.MaxOpenGenericRequestParamCount = 1;
                    cfg.RegisterServicesFromAssembly(assembly);
                    cfg.RegisterGenericHandlers = true;
                });
            })
        ...
        public void ShouldThrowExceptionWhenTimeoutOccurs()
        {
            ...
            Should.Throw<TimeoutException>(() =>
            {
                services.AddFlowR(cfg =>
                {
                    cfg.MaxOpenGenericRequestParamCount = 0;
                    cfg.MaxOpenGenericHandlerCount = 0;
                    cfg.MaxOpenGenericRequestTypeCount = 0;
                    cfg.GenericRegistrationTimeout = 1000;
                    cfg.RegisterGenericHandlers = true;
                    cfg.RegisterServicesFromAssembly(assembly);
                });
            })
        ...
        [Fact]
        public void TestRegistrationTimeoutAcceptance()
        {
            var config = new FlowRServiceConfiguration();

            config.GenericRegistrationTimeout = 1000;
            config.GenericRegistrationTimeout.ShouldBe(1000);

            config.MaxOpenGenericRequestParamCount = 10;
            config.MaxOpenGenericRequestParamCount.ShouldBe(10);

            config.MaxOpenGenericHandlerCount = 25;
            config.MaxOpenGenericHandlerCount.ShouldBe(25);

            config.MaxOpenGenericRequestTypeCount = 50;
            config.MaxOpenGenericRequestTypeCount.ShouldBe(50);
        }
    }
}
